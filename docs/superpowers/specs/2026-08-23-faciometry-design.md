# Faciometry — open-source facial morphometrics, design
...
Faciometry is a locally-hosted, open-source system that takes standardised photographs
...
Faciometry does not emit a scalar attractiveness score, rating, rank, or "harmony
...
Faciometry also does not do identification, matching, or demographic inference as a
...
1. `faciometry analyze front.jpg --profile side.jpg` produces a report in < 10 s on
...
require a reference; Faciometry uses interpupillary distance against a sex-stratified
...
substantially at close range. Faciometry estimates effective focal length from EXIF
...
Landmark backends emit heatmaps; Faciometry converts each to a per-point 2×2
...
A separately-versioned artifact (`faciometry-norms-<version>.parquet`) mapping
...
- If the subject's stratum has `n` below threshold, Faciometry falls back to the
...
- Weights and norms fetched on first run from Hugging Face into
  `~/.cache/faciometry/`, each pinned by sha256 recorded in `assets/weights.lock.json`.
  A mismatch is a hard failure.
- `faciometry serve` → FastAPI on 127.0.0.1 with a local web UI (upload, capture
  guidance overlay, report view, export). Binds loopback only by default.
- `faciometry analyze` → CLI, JSON + HTML + PDF output.
...
gate is therefore at the edge of resolvability. Faciometry gates on
...
Faciometry keeps the measurements, the reference ranges as context, and the
