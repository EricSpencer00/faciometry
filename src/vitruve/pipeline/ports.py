...
* The seam is documented in one file. When a member below turns out to already
  exist under a different name in ``vitruve.models.protocols``, deleting the
...
**subject's**, matching :mod:`vitruve.core.landmarks`. In an unmirrored frontal
...
class Backends:
    ...
    def check_licences(self, allowed: Tier) -> None:
        ...
        bundle handed to :func:`vitruve.pipeline.run.analyze` is the bundle the
...
def load_backends(*, tier: Tier, device: str | None = None) -> Backends:
    """Build the default backend bundle from :mod:`vitruve.models`.
    ...
    try:
        ...
        raise RuntimeError(
            "no model backends are installed. Vitruve's measurement layer runs "
