...
environment override exists (`VITRUVE_DEVICE`) because reproducing a reported
...
float64 dtype` the moment one appears. This collides directly with Vitruve's
...
def resolve(requested: str | None = None, *, env: dict[str, str] | None = None) -> Device:
    """Pick a device, honouring an explicit request or `VITRUVE_DEVICE`.
    ...
    if choice is not None:
        ...
        if kind not in VALID:
            raise DeviceUnavailable(
                f"{kind!r} is not a device Vitruve knows; expected one of {sorted(VALID)}"
            )
...
def pin_to_cpu(requested: Device | None, reason: str) -> tuple[Device, str]:
    ...
    Two of the four backends Vitruve loads are pinned this way, and for the same
