...
recorded for it. Backends that Vitruve knows about but cannot load are listed
...
class Entry:
    ...
    #: `import vitruve.models.registry` stays free of torch and mediapipe.
    ...
    def factory(self) -> Factory:
        if self.module is None:
            raise BackendUnavailable(
                f"{self.name} is known to Vitruve but not implemented here: "
...
LANDMARKERS: dict[str, Entry] = {
    ...
    "star": Entry(
        ...
        unavailable_reason=(
            ...
            "all, so all rights are reserved and there is no tier at which Vitruve "
            "will load it"
        ),
    ),
}
...
def describe(tier: Tier = Tier.PERMISSIVE) -> tuple[str, ...]:
    ...
    Written for `vitruve backends`, so a user comparing tiers can see what
