...
* **Uncertainty is anisotropic and comes from the model.** `heatmap_moments`
  ...
  what the rest of Vitruve is built to avoid.
...
class DenseResult:
    ...
    they are the only permissively-licensed metric cue Vitruve has:
    :func:`vitruve.core.scale.from_iris` turns a horizontal iris diameter in
    ...
    constant-scale assumption that Vitruve does not use.
    """
...
class HeadPose:
    ...
    def disagreement(self, other: HeadPose) -> float:
        ...
        Vitruve carries a second pose model at all.
        """
        return max(
