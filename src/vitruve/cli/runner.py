...
timestamp and, on a phone, a GPS fix. Vitruve decodes to a pixel array and
...
**The pipeline is discovered, not imported.** ``vitruve.pipeline`` and
``vitruve.report`` are built separately from the transports, and a transport
...
class AnalysisRequest:
    ...
    They are declared by the subject or left empty; Vitruve never predicts
...
def _pipeline_module() -> Any:
    try:
        return importlib.import_module("vitruve.pipeline")
    except ImportError as exc:
        raise PipelineUnavailable(
            "no analysis pipeline in this build: `import vitruve.pipeline` failed "
            f"({exc}). The measurement layer is independent of it, so "
            "`vitruve catalogue`, `vitruve licenses` and `vitruve doctor` still work."
        ) from exc
...
def _entry_point() -> tuple[str, Any]:
    ...
    for name in PATH_ENTRY_POINTS:
        ...
        if callable(fn):
            return "path", fn
    raise PipelineUnavailable(
        "vitruve.pipeline imported but exposes none of "
...
def _fallback_text(outcome: AnalysisOutcome) -> str:
    ...
    lines = ["Vitruve report", "=" * 60, "", f"REPORTED ({len(shown)})"]
