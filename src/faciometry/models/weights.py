...
* :func:`resolve` looks a weight up in the cache, verifies its sha256, and
  ...
  exists to forbid.
* :func:`download` fetches. It is called by `faciometry fetch-weights` and by
...
CACHE_ENV_VAR = "FACIOMETRY_CACHE_DIR"
DEFAULT_CACHE = Path.home() / ".cache" / "faciometry"

#: Where the pins live. Overridable mainly so tests can pin against a fixture.
LOCK_ENV_VAR = "FACIOMETRY_WEIGHTS_LOCK"
...
#: Installed, `src/faciometry/models/weights.py` becomes
#: `site-packages/faciometry/models/weights.py`, so the same expression points at
...
def load_lock(path: Path | None = None) -> dict[str, WeightSpec]:
    ...
    Not cached at module level: a test that points `FACIOMETRY_WEIGHTS_LOCK` at a
...
def verify_only(key: str, *, lock: Mapping[str, WeightSpec] | None = None) -> VerifyResult:
    ...
    Returns a result rather than raising, so `faciometry weights verify` can report
...
def resolve(key: str, *, lock: Mapping[str, WeightSpec] | None = None) -> Path:
    ...
    if not result.present:
        raise WeightsUnavailable(
            ...
            f"    faciometry fetch-weights\n"
            f"    (or python -m faciometry.models.weights fetch {key})\n"
            f"Source: {spec.url}"
        )
...
def download(
    ...
    *,
    ...
    tmp = Path(tmp_name)
    try:
        request = urllib.request.Request(spec.url, headers={"User-Agent": "faciometry/weights"})
...
def _main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="faciometry.models.weights")
