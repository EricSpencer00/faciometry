"""`faciometry fetch-weights`: the only command that opens a socket.

The pinning, hashing and atomic-rename logic lives in
:mod:`faciometry.models.weights`, next to the backends that consume it. This
...
Analysis never calls anything in here. `faciometry.models.weights.resolve` reads
...
def _module():
    try:
        ...
        raise WeightsLayerMissing(
            "no faciometry.models in this build, so no weights are pinned. The "
            "measurement layer runs without them; `faciometry catalogue` and "
            "`faciometry licenses` do not need a single byte of model."
        ) from exc
...
def run(tier: Tier = Tier.PERMISSIVE, *, force: bool = False) -> int:
    try:
        ...
        print(f"faciometry fetch-weights: {exc}", file=sys.stderr)
        return 2
