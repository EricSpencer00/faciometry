...
``faciometry.measure.registry.CATALOGUE``.
"""
...
#: The pose Faciometry quotes sensitivity at: ten degrees on each of yaw, pitch
...
def render_table(selected: tuple[Row, ...]) -> str:
    ...
    out: list[str] = []
    out.append(f"Faciometry measurement catalogue: {len(selected)} of {len(CATALOGUE)} measurements")
    out.append("")
    ...
    out.extend(
        ...
        for line in (
            ...
            "carries more person than photograph. Below 1 Faciometry withholds it.",
        )
    )
    out.append("")
    ...
    *,
    ...
    if spec_id is not None:
        if spec_id not in BY_ID:
            print(f"no measurement with id {spec_id!r}. Run `faciometry catalogue` for the list.")
            return 2
        if as_json:
