...
def run(
    *,
    ...
    try:
        ...
        print(f"faciometry analyze: {exc}", file=sys.stderr)
        return Exit.BAD_INPUT

    if license_tier > Tier.PERMISSIVE:
        print(
            f"faciometry analyze: license tier {license_tier.name.lower()} selected. "
            f"Run `faciometry licenses --tier {license_tier.name.lower()}` for what that "
            "obliges you to.",
            file=sys.stderr,
        )
    ...
    if outcome.status is Status.OK:
        ...
        if out:
            ...
            print(
                "faciometry analyze: --pdf needs --out, because a PDF is a file "
    ...
    print(f"faciometry analyze: {outcome.message}", file=sys.stderr)
...
def _write_pdf(directory: Path, outcome) -> int:
    ...
    try:
        ...
        print(f"faciometry analyze: the PDF report is unavailable ({exc})", file=sys.stderr)
        return Exit.ERROR
    try:
