...
replaced upstream changes every number Faciometry prints and leaves no trace in
...
def resolve_device() -> tuple[str, str]:
    ...
    Deferred to :mod:`faciometry.models.device` when that layer is installed, so
    the device `doctor` prints is the device an analysis will actually get,
    including a ``FACIOMETRY_DEVICE`` override. Falling back to a local guess
...
def collect(tier: Tier = Tier.PERMISSIVE) -> list[Check]:
    ...
    checks: list[Check] = [
        Check(OK, "faciometry", __version__),
        Check(
    ...
    for module_name, dist in (("fastapi", "fastapi"), ("uvicorn", "uvicorn")):
        ...
        checks.append(
            ...
            if version
            else Check(WARN, dist, "not installed; `faciometry serve` needs faciometry[api]")
        )

    checks.append(
        Check(OK, "pipeline", "importable")
        if analysis_available()
        else Check(WARN, "pipeline", "no faciometry.pipeline in this build, so analysis will not run")
    )
...
def _weight_checks(tier: Tier) -> list[Check]:
    ...
    number Faciometry prints and leaves nothing in the output to show it, so the
    ...
    for result in results:
        if result.ok:
            ...
            out.append(Check(WARN, result.key, "not fetched; run `faciometry fetch-weights`"))
        else:
            out.append(
                Check(
                    FAIL,
