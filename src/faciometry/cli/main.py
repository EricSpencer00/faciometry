...
and `faciometry catalogue`, which is a command that has to work on a machine with
nothing installed.
...
EPILOG = """\
examples:
  faciometry catalogue                        every measurement and how well each survives a photograph
  faciometry catalogue --id gonial_angle_l    everything known about one measurement
  faciometry licenses --tier copyleft         what the AGPL tier obliges you to
  faciometry doctor                           device, weights, versions
  faciometry fetch-weights                    the only command that uses the network
  faciometry analyze front.jpg --profile side.jpg --out report/
  faciometry analyze a.jpg b.jpg c.jpg --out report/   pool three captures of one face
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"faciometry {__version__}")
    sub = parser.add_subparsers(dest="command", metavar="command")

    analyze = sub.add_parser(
        "analyze",
        help="measure a face from a photograph",
        description="Measure a face. Nothing is downloaded; run `faciometry fetch-weights` first.",
    )
    ...
    analyze.add_argument(
        "--pdf",
        action="store_true",
        help="also write report.pdf, the typeset report (needs pip install 'faciometry[pdf]')",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "serve":
        ...
        try:
            ...
            print(f"faciometry serve: {exc}", file=sys.stderr)
            return Exit.BAD_INPUT
        except ImportError as exc:
            print(
                f"faciometry serve: {exc}. Install the server with "
                "`pip install 'faciometry[api]'`.",
                file=sys.stderr,
            )
            return Exit.ERROR
