...
to listen on. Faciometry handles photographs of faces and runs with no
...
operator who genuinely wants Faciometry on a lab subnet behind their own auth
...
WARNING = """\
==============================================================================
  faciometry is binding {host}, which is reachable from the network.
...
def resolve_bind(host: str, *, allow_remote: bool) -> str:
    ...
    if not allow_remote:
        raise RemoteBindRefused(
            f"refusing to bind {host}: it is reachable from the network and Faciometry "
...
def serve(
    *,
    ...
    if store:
        print(
            f"faciometry serve: --store is on, so uploaded images are written to "
            f"{Path(store_dir) if store_dir else Path('faciometry-store')} with their "
            "metadata stripped.",
            file=sys.stderr,
        )

    app = create_app(store=store, store_dir=store_dir, license_tier=license_tier)
    print(f"faciometry serve: http://{bind}:{port}", file=sys.stderr)
