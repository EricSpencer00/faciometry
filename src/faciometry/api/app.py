...
**It binds loopback.** ``faciometry serve`` refuses a non-loopback address unless
...
def web_root() -> Path | None:
    ...
    override = os.environ.get("FACIOMETRY_WEB")
    if override:
        p = Path(override)
...
def create_app(
    *,
    ...
    app.state.store_dir = Path(store_dir) if store_dir else Path("faciometry-store")
