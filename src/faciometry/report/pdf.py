...
install of Faciometry is ``numpy + pillow`` and has to stay that way.
...
imports ``faciometry.report`` and only fails when a PDF is actually asked for.
...
Two runs over the same :class:`~faciometry.report.model.ReportInput` produce
...
INSTALL_HINT = "pip install 'faciometry[pdf]'   (or: pip install reportlab)"
...
HOW_TO_READ: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "landmark model does not supply a point the formula reads. Faciometry does "
    ...
    (
        ...
        "and it is not a target; Faciometry does not hold a face against it.",
    ),
    (
        "Provenance",
...
METHOD: tuple[str, ...] = (
    ...
    "than anything shared (Hoenekopp 2006). Faciometry reports measurements. It "
...
#:
#:     from faciometry.report.composite import mirror_composites
...
def skin_findings(report: ReportInput, given: Any = None) -> tuple[Any, ...]:
    ...
    ``faciometry.derm`` is optional and ``ReportInput`` has no field for it yet,
...
def draw_cover(sheet: Sheet, cover: Cover) -> None:
    ...
    y = PAGE_H - 96.0
    draw_tracked(
        c, MARGIN, y, "FACIOMETRY", font=SERIF, size=30.0, tracking=9.0, color=INK
    )
    c.setStrokeColor(HexColor(INK))
    c.setLineWidth(1.4)
...
def render_pdf_bytes(
    ...
    *,
    ...
    ``faciometry.derm`` ``FindingSet``, or anything iterable of findings that can
    format themselves.
    """
    try:
    ...
    c.setTitle(_flat(f"Faciometry morphometric report, {report.subject_label}"))
    c.setSubject(
        _flat(
    ...
    c.setAuthor("Faciometry")
    c.setCreator("Faciometry")
    c.setProducer("Faciometry (reportlab)")
    sheet = Sheet(
        c,
        subject=report.subject_label or "Unidentified subject",
        generated_at=report.generated_at or "date not recorded",
